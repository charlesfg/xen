...
long do_arbitrary_access(unsigned long dst_maddr,  void *buff, size_t n, int action)
{
    mfn_t mfn;
    void *va_dst_maddr, *d;
    ...
    {
        rb = __raw_copy_to_guest(buff, d, n);
        if (rb)
        {
            LOG("Residual bytes to copy to user:  %ld bytes", rb);
            rb = __raw_copy_to_guest(buff+(n-rb), d+(n-rb), rb);
            if (rb)
    ...
    {
        rb = __raw_copy_from_guest(d, buff, n);
        if (rb)
        {
            LOG("Residual bytes to copy from user:  %ld bytes", rb);
            rb = __raw_copy_from_guest(d+(n-rb), buff+(n-rb), n);
            if (rb)
                LOG("Could not copy %ld bytes", rb);
        }
        LOG("Done!"); 
